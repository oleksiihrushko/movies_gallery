...
const renderFavorites = galleryItems => {
  ...
  galleryItems.forEach(item => {
    if (lsData.includes(String(item.id))) {
      markup += `<div class="favorites__item">
        <span class="favorites__item--name">${item.name}</span>
        <button data-itemid=${item.id} class="favorites__item--btn">X</button>
      </div>`;
    }
  });
  ...
  delBtns.forEach(btn =>
    btn.addEventListener('click', e => {
      btnClickHandler(e.currentTarget.dataset.itemid, galleryItems);
    }),
  );
};
